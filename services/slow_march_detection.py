...
cursor = conn.cursor()
cursor.execute("""
    CREATE TABLE IF NOT EXISTS slow_march_result (
    ...
    while cap.isOpened():
        ...
        try:
            if results.pose_landmarks:
                ...
                screenshot_path = f"static/screenshots/slow_march_{int(time.time())}.jpg"
                status = "slow_march wrong"
                ...
                if left_z < Z_THRESHOLD and right_wrist_z < Z_THRESHOLD:
                    ...
                    cursor.execute("INSERT INTO slow_march_result (timestamp, angle, status, suggestion, screenshot_path) VALUES (?, ?, ?, ?, ?)",
